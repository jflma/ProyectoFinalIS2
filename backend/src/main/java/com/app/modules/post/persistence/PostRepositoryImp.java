...
public interface PostRepositoryImp extends JpaRepository<Post, Long> {
  ...
  @Query("SELECT new com.app.controller.dto.response.PostPreviewDTO(p.id, p.title, p.views, p.answers, e.content, e.upVotes, e.downVotes)"
      + "FROM Post p JOIN p.entry e ORDER BY p.entry.createdAt DESC")
  public List<PostPreviewDTO> findTop10ByOrderByEntryCreatedAt();

  @Query("SELECT new com.app.controller.dto.response.PostDetailsDTO(p.id, e.id, p.title, e.content, e.createdAt, u.username)"
