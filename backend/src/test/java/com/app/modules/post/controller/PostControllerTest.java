...
public class PostControllerTest extends BaseIntegrationTest {
        ...
        void testCreatePostUnauthorized() throws Exception {
                ...
                mockMvc.perform(post("/post/create")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(fields)))
                                .andExpect(status().isUnauthorized()); // Spring Security default for missing auth is
                                                                       // 403 or 401 depending
                                                                       // on config. Usually 403 Forbidden effectively
                                                                       // prevents access.
                                                                       // However, request asked for 401. If config is
                                                                       // standard, missing
                                                                       // token might be 401 or 403. Let's check.
                                                                       // If default entry point is 403, we might need
                                                                       // to adjust
                                                                       // expectation or config.
                                                                       // For now, I'll expect status().isUnauthorized()
                                                                       // OR isForbidden()
                                                                       // is safer if not fully specified, but Request
                                                                       // said "retorna 401".
                                                                       // Without custom EntryPoint, Spring Security
                                                                       // typically returns 403
                                                                       // Forbidden for non-authenticated access to
                                                                       // protected resource if
                                                                       // not configured otherwise for REST.
                                                                       // Let me check if I can enforce 401 expectation.
                                                                       // Actually, if permitAll is not set and no
                                                                       // token, it is 403
                                                                       // Forbidden (Access Denied) or 401
